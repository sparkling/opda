...
* Neutral, because the Artalk task runs on **Fargate Spot** (~$2.7/mo, comparable to the Fly machine it replaces) rather than on-demand (~$7–9/mo): a Spot reclaim is a 2-minute-warned stop followed by auto-relaunch — operationally identical to a normal deploy (Litestream restore + origin-DNS repoint) — so its only cost is brief, rare comment downtime, acceptable at this scale. App Runner (request-scaled toward zero) was rejected because its rolling deploys overlap instances, violating the single-writer invariant; a small EC2 instance was rejected for reintroducing VM patching.
