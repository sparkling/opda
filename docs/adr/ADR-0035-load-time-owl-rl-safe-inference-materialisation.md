...
* **B — Live Jena `ja:reasoner` (InfModel) assembler** — a `config/opda-fuseki-assembler.ttl` with a `GenericRuleReasoner` reading `config/opda-rdfs-plus.rules`, computing entailment in-memory at query time.
...
* Bad, because it adds a governed config file (`config/opda-rdfs-plus.rules`) and load-step surface that must be kept in sync with ODR-0025 R1/R2.
...
### 2026-06-14 — Council session-039 reframe: minimal/dormant, honest naming, frozen rule logic

Council [session-039](../ontology/odr/council/session-039-entailment-regime-and-epc-modelling.md) reviewed this regime against an operator suspicion that the bespoke subset masks modelling errors. Outcome (full disposition in [ODR-0029](../ontology/odr/ODR-0029-inference-validation-boundary-and-entailment-regime-disposition.md)): the regime is **sound and stays**, reframed — not reduced. **Rule logic is unchanged** (operator constraint: no rule-body / enabled-set edits — renaming and re-documenting are permitted); these are documentation/framing amendments only, and `ci-inference-closure` byte-identity is the guard that nothing semantic moved.

* **Minimal/dormant, not a rich reasoner.** Verified against `ci/inference_closure_test.py`: of the 7 Safe-Group rules, **only `rdfs:subClassOf` type-propagation currently fires** — the corpus declares zero `owl:inverseOf`/`Transitive`/`Symmetric`/`disjointWith` and the hierarchy is flat (ODR-0027). Closure adds **0 triples** over the schema, ~30 (one-hop, query-recoverable) over the exemplars. The other six rules are **dormant carriers** that activate when a real construct lands. Describe this as a *declared entailment boundary currently exercised by one rule*, not a rich reasoning layer (avoids implying semantics the corpus lacks).
* **The negative consistency gate is the high-value asset.** The guarantee that the closure introduces *no* `owl:sameAs` and *no* spurious `EPCCertificate ⊑ Property` is retained and is the main thing the layer buys — it stands independent of the (near-empty) positive closure.
* **Honest naming (rename permitted, logic frozen).** The ruleset is a **sound, RL-incomplete** fragment — removing rules from a sound set reduces *completeness*, never soundness — and is **NOT an OWL 2 RL reasoner**; its output is not the OWL 2 RL closure. The former `config/opda-owl-rl-safe.rules` + "OWL 2 RL Safe Group" header over-claimed conformance; renamed to `config/opda-rdfs-plus.rules` ("RDFS-Plus"; ODR-0029 R5) with a SOUND-but-INTENTIONALLY-INCOMPLETE header, keeping "model-but-don't-evaluate" (ODR-0026 R2) verbatim. **No rule body changes.**
* **EPC defect is edge-fixes, not a regime change.** The EPC cross-trip (ODR-0028 R3) is *not* produced by this regime — the Safe Group's exclusion of `domain`/`range` already prevents it (ODR-0025 §R7). It arises only under the ADR-0014 round-trip's full `inference="rdfs"` flag, *wider* than this regime. Fix: point the round-trip at the materialised Safe-Group closure (not full RDFS); resolve the dangling `Baspi5_EPCCertificateShape`; add the `domain`/`range`-as-SHACL-constraint layer (ODR-0029 R3) so domain mismatches are *validated*, not silently uncaught. None of these touch the entailment rules.

Net: keep the materialiser and its rules as-is (logic frozen); rename + re-document for honesty; reframe the narrative as minimal/dormant; route `domain`/`range` checking to SHACL.
