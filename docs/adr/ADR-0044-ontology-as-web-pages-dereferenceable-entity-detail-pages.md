...
* **Implementation status (2026-06-15): Phase 1 shipped.** `scripts/ontology-model.mjs` + committed `src/data/ontology-model.json` (deterministic; incoming+outgoing, attributes, resolved SHACL, SKOS, contexts, `dct:source`), wired into `build:data`. Phases 2–8 pending. (Uncommitted working tree at time of writing — see [HANDOVER-2026-06-15](../HANDOVER-2026-06-15-ontology-web-pages-phase1-and-namespace-api-fix.md).)
...
1. **Model extraction — ✅ DONE (2026-06-15).** `scripts/ontology-model.mjs` queries live Fuseki → committed **deterministic** `src/data/ontology-model.json` (incoming+outgoing, datatype attributes, blank-node-resolved SHACL constraints, SKOS hierarchy, scheme/context membership, `dct:source`); wired into `build:data` step 3.5 + `make ontology-model` / `npm run ontology:model`. Validated against the corpus: 40 classes · 30 obj + 226 data props · 325 shapes · 314 concepts · 48 schemes; byte-identical re-run (gate-ready); `Property` shows both incoming **and** outgoing. Shared with the ADR-0043 `elements.json` (same extraction). *Refinement for phase 3: `contexts` currently includes the cross-cutting `shapes`/`vocabularies`/`contexts` graphs — the context pages must filter to the canonical 7 bounded contexts.*
2. **Class + Property detail pages** (the core ask) — routes + components + incoming/outgoing + cross-links. *Verify: `opda:Property` shows both directions.*
3. **Shape + Concept + Scheme + Context pages.** *Verify: blank-node-free shape rendering; concept tree links resolve.*
4. **Aggregate pages → typed indexes**; retire the regex parsers; A-Z index links every term. *Verify: no regex parsing remains; index complete.*
5. **Other pages** — profiles, exemplars, UFO categories, namespaces, datatypes. *Verify: profile gap register renders; categories group classes.*
6. **Dereferenceability (pure SSG)** — `/pdtf/[name]` HTML page + `/pdtf/[name].ttl` static endpoint + `rel="alternate"`; `build.format: 'file'` for clean IRI resolution. *Verify: bare IRI serves HTML, `.ttl` serves Turtle, no trailing-slash redirect. (Optional later: edge `Accept`-conneg on the bare IRI.)*
7. **Doc-drift gate** — regenerate + diff `ontology-model.json`. *Verify: CI fails on drift.*
8. **Validate all links & fix all errors (final sweep).** Crawl every generated page; validate internal cross-links (no dangling/orphan), external URLs (200, cached/allow-listed), and render/structure (no empty section, unresolved blank node, malformed CBD, broken anchor). **Fix every error, re-run until clean.** *Verify: zero broken links, zero errors; the gate is part of `make ci` and blocks the deploy.*
