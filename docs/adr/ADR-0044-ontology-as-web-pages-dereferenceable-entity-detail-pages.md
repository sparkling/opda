...
5. **Other pages — ✅ DONE (UFO categories deferred).** namespaces, datatypes, per-profile and per-exemplar pages shipped. **UFO meta-category pages are deferred:** classes carry their UFO category only as free-text in `skos:scopeNote` / `rdfs:comment`, not as a structured term — category pages need a structured class-level `opda:ufoCategory` in the corpus first (an ontology-modelling change), not a fragile free-text parse.
