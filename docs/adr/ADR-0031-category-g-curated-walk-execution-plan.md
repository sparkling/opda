...
* **Outcome (2026-05-31):** this walk executed (185/188 candidate-G emitted-or-collapsed) and was reviewed by Council [session-028](../ontology/odr/council/session-028-category-g-walk-review.md), ratified as [ODR-0024](../ontology/odr/ODR-0024-curated-category-g-walk-dispositions.md) (the modelling rules) + [ADR-0032](./ADR-0032-category-g-walk-emission-and-coverage-gate.md) (the engineering + the two CI gates `ci-dup-declaration` / `ci-category-g-coverage`), and remediated to an honest **179/239** coverage. The leftover — the monetary walk (**executed 2026-05-31, §G22 closed** — `opda:MonetaryAmount` + ISO-4217 `opda:CurrencyScheme` + 16 per-bearer monetary properties; coverage 179→196/239), the R5-surfaced follow-on (**executed 2026-05-31, §G23 closed** — 40 enum/Yes-No leaves minted + the 5 R6 schemes + `ownerType` wired; coverage 196→**236/239**), and the held `opda:Room` leaves — is tracked in [ADR-0005](./ADR-0005-deferred-work-register.md) §G22–G24. **With G22 + G23 + G24 closed, the curated Category-G walk is COMPLETE at 239/239** — the held `opda:Room`/`opda:Building` leaves (§G24) were modelled as the anonymous by-value `opda:RoomDimension` structure via [ADR-0033](./ADR-0033-room-dimension-value-structure-emission.md) (Council session-030; classes not minted — neither has a data-realisable IC).
* **R2 sequencing closed (2026-05-31):** work-item-4's sequence (walk → R2 convenes → if load-bearing, spawn ODR-0008a/b/c) ran to completion. R2 convened — Council [session-029](../ontology/odr/council/session-029-r2-ufo-axis-load-bearing.md) — and judged the UFO-axis split **NOT operationally load-bearing (0–6–0)**: the walk produced a *bearer-domain* (`rdfs:domain`) partition, not the UFO-typed one §Q2a(a) presupposed, so **ODR-0008a/b/c were NOT spawned** (ODR-0023 R2 updated; the gUFO typing pass that would build the typed set is deferred as [ADR-0005](./ADR-0005-deferred-work-register.md) §G25).
