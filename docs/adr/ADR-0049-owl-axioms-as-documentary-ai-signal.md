...
4. ~~**Adopt hm ODR-0030** permitted/excluded construct table + the excluded-construct CI meta-shape…~~ **DONE (2026-06-17, commit `8681c4c`):** permitted/excluded OWL-construct table added as **ODR-0033 §R6** (3 bands re-keyed to OPDA's frozen closure); corpus-wide **`ci-excluded-construct`** gate (wired into `make ci-ontology`) generalising the object-property b3 limb to all 9 excluded families corpus-wide (positive control fires; corpus has zero → passes, arms drift protection). **Decision-4 gates landed in the same commit:** `ci-description-coverage` (`skos:definition`) + `ci-isDefinedBy` (`rdfs:isDefinedBy`), both wired **warning-first**. Initially both reported 0/303 (warning-first). **BACKFILL DONE (2026-06-18, commit `d937374`):** fanned-out per-module drafting added an ISO-704 genus–differentia `skos:definition`@en + `rdfs:isDefinedBy` → `owl:Ontology` to **all 303 OWL terms** (sourced from the PDTF v3 JSON-schema descriptions + each term's existing `rdfs:comment`/`scopeNote` + domain). **Both gates now 303/303 (100%) and have FLIPPED warning-first → hard** (a future term without a definition/isDefinedBy fails CI). The gate loader gained `foundation.ttl` so the base `owl:Ontology` is recognised for the foundation classes; `consumesFrom` was defined + repointed to base; the baseline unit test was updated to the completed state. byte-identity re-pinned; full `make ci` green; no model/graph-JSON drift.
...
6. ~~**`scripts/ontology-model.mjs:207-248`** SHACL-derivation block…~~ **DONE (2026-06-17): KEEP as belt-and-braces** (session-050 Q4). The object properties now carry documentary domain/range so single-class edges derive natively; the SHACL recovery is retained (not reverted) because the disjunctive "any-of" properties carry authoritative per-class typing only in SHACL `sh:or` (documentary multi-domain/range under-determines the precise per-class edge), and the merge is idempotent with the native edges. Comment corrected (the superseded session-047 "kept out of OWL to avoid mis-entailment" premise replaced); no code/output change.
