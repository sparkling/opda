...
| G5 | Mirror Section G to `/governance/deferred-work` Astro page | Technical WG / next contributor touching the mirror | Before next quarterly publish OR when next Section G item lands | Open (queued 2026-05-27) | The Astro mirror at `src/pages/governance/deferred-work.astro` has columns Status + Linked artefact; Section G needs a corresponding table. Mechanical sync work; not a blocker for the ADR programme. |
| G6 | Cosmetic ADR-0009 template amendment — document pinning convention | Author / opportunistic | Opportunistic | Open (queued 2026-05-27 from ADR-0009 validation) | ADR-0009 §"foundation.ttl" template (lines 49-50) uses placeholder syntax `# Source commit: <git-sha>` and `dct:modified "<emission-date>"^^xsd:date`. Worker correctly pinned these to sentinel constants (`"pinned-by-ADR-0009"` and `2026-05-27`) because live HEAD SHA / live date silently break byte-identity CI on every unrelated commit. Within-engineering decision (per programme plan §9.4: does NOT require Council route — `## Rules` are unaffected). Amend the template prose to document the pinning convention so future-readers don't misinterpret the angle-bracket placeholders as live-substitution directives. |
...
| G8 | ADR-0010 scope expansion — additional SKOS schemes beyond the named first batch | Worker on first downstream consumer requesting an unemitted scheme | When a downstream module ADR (ADR-0011+) cites a scheme not in the ADR-0010 §"Scheme catalogue" 16-row table | Open (queued 2026-05-27 from ADR-0010 implementation) | ADR-0010 emits the named first batch only (16 schemes covering ~88 concepts). The data dictionary carries ~1225 leaves with top-level enums; many are out-of-scope for the BASPI5 MVP but become relevant for downstream overlays (TA6/NTS/LPE1). Expansion happens scheme-by-scheme as downstream demand surfaces, not as a one-shot programme. Each expansion follows ADR-0010's per-scheme metadata MUST-haves; A9 per-kind discipline (UFO category + IC + artefact realisation) applies. |
| G9 | Populate data-dictionary enums for MilestoneKind / AssuranceLevel / EvidenceMethod / AddressVariant | Data dictionary maintainer | Opportunistic (next dictionary refresh cycle) | Open (queued 2026-05-27 from ADR-0010 implementation) | Four ADR-0010 schemes (MilestoneKindScheme, AssuranceLevelScheme, EvidenceMethodScheme, AddressVariantScheme) are conceptually well-defined by external standards (PDTF process, eIDAS, OIDC4IDA, ODR-0015 §S015 Q1) but have no corresponding enum leaves in `data-dictionary-canonical.json`. They were emitted with `PLACEHOLDER:` warnings inside `skos:scopeNote @en` per the ADR-0010 implementation brief's option (a) routing. When the data dictionary populates these enums, regenerate the schemes pointing per-member `dct:source` at the new leaf paths and remove the PLACEHOLDER text. |
| G10 | TransactionStatusScheme per-member `dct:source` URI fabrication audit | Worker on next vocabularies regeneration | Opportunistic, MUST land before ADR-0014 BASPI5 round-trip (because BASPI5 needs full `dct:source` traceability) | Open (queued 2026-05-27 from ADR-0010 validation) | The 5 TransactionStatus members (`Listed / Offered / Accepted / Exchanged / Completed`) emit per-member `dct:source` URIs like `<https://w3id.org/opda/data-dictionary#status.Listed>` — but those notations do NOT appear in the data-dictionary `status` enum (which has 9 values: `'active', 'For sale', 'Under offer', 'Sold subject to contract', 'Contracts exchanged', 'Completed', 'Cancelled', 'To let', 'Let agreed'`). The URIs are fabricated fragments. Three remediation options (validator's recommendation): **(a)** cite ODR-0011 §8a instead of the fabricated data-dict fragment; **(b)** cite the actual data-dict enum value the canonical label sources from (e.g. `Listed ← 'For sale'` via per-member `dct:source` + `prov:wasDerivedFrom`); **(c)** mint a new `<https://w3id.org/opda/transaction-status#Listed>` namespace for the UFO-canonical 5-set. Engineering choice, not Council route (ODR-0011 §Rules.3 says "citation to origin" — origin is engineering-determined). |
| G11 | Full Q5a binding-table emission (~44 leaves) into opda-property.ttl + opda-descriptive.ttl | Next downstream module ADR or overlay-profile worker (ADR-0013+) | Per-leaf as each downstream consumer requests a specific descriptive attribute (BASPI5 round-trip will surface the first batch) | Open (queued 2026-05-27 from ADR-0011 implementation) | ADR-0011 emits the minimum binding-table subset needed by the 15 diagnostic exemplars + the core opda-property.ttl identity predicates: `builtForm`, `currentEnergyRating`, `tenureKind`, `addressVariant`, `hasUPRN`, `hasAddress` (6 of ~50). The remaining ~44 leaves (`councilTaxBand`, `centralHeatingFuelType`, `heatingType`, `ownershipType`, `priceQualifier`, `marketingTenure`, `yesNoNotKnown`-family flags, lexical patterns, free-text strings, etc.) emit per ODR-0008 §Q5a table when a downstream module or overlay needs them. Engineering choice per-leaf — UFO category + SHACL modelling are named by §Q5a; the worker applies them without Council deliberation. A9 per-kind discipline (dct:source to data-dictionary leaf + skos:scopeNote where SKOS-bound) applies per leaf. |
