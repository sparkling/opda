...
| G7 | Prefix-filter edge cases for embedded URLs in rdfs:comment / dct:description | Worker on first ADR-0011 module emission | When first `rdfs:comment` containing a URL is emitted as a Literal | Open (queued 2026-05-27 from ADR-0009 validation) | G2's fix scans Literal lexical values for IRI-shaped strings that match bound namespaces. Foundation TTLs do not currently emit URL-bearing rdfs:comment Literals — but ADR-0011 module classes typically will. Add a regression test exercising: (a) a Literal containing a URL in a bound namespace (should NOT add a spurious prefix binding); (b) a Literal containing a URL in a non-bound namespace (should NOT bind a new prefix). The current filter is conservative-safe; the regression test cements the contract. |
