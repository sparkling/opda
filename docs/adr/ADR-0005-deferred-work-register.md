...
| G11 | Full Q5a binding-table emission (~44 leaves) into opda-property.ttl + opda-descriptive.ttl | ADR-0013 worker (bundled with overlay profile emission) | Closure tightened per ADR-0011 validator: **BEFORE ADR-0014 MVP gate** — every leaf BASPI5 round-trip discovers as missing MUST emit, no silent omissions | Bundled with ADR-0013 worker brief | ADR-0011 emits the minimum binding-table subset needed by the 15 diagnostic exemplars + the core opda-property.ttl identity predicates: `builtForm`, `currentEnergyRating`, `tenureKind`, `addressVariant`, `hasUPRN`, `hasAddress` (6 of ~50). ADR-0013 worker emits every additional leaf BASPI5 binds to (per the 334 enum-bearing leaves in `baspi5.json`); ADR-0014 MVP gate fails if any required leaf is missing. Engineering choice per-leaf per ODR-0008 §Q5a — UFO category + SHACL modelling are named in the table. A9 per-kind discipline (dct:source to data-dictionary leaf + skos:scopeNote where SKOS-bound) applies per leaf. |
| G12 | Canonical serialiser blank-node deduplication for multi-target shared blank nodes | ADR-0013 worker (bundled) | Cosmetic; opportunistic | Bundled with ADR-0013 worker brief | ADR-0012 descriptive shapes emit one shared `sh:property` blank node referenced by 5 NodeShapes (Survey/EPCCertificate/Search/Valuation/Comparable, each targeting `prov:wasGeneratedBy`). The canonical serialiser correctly emits semantically-equivalent output but renders the blank-node block 5 times rather than naming it once. Cosmetic improvement; semantically equivalent. Worker can either inline the shared-blank logic into a named meta-shape OR teach the serialiser to detect shared-blank-node references. |
