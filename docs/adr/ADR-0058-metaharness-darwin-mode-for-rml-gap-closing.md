...
- **Net correction (as of the first re-review pass)**: the *mechanism* can, in principle, target arbitrary repo files including this ontology — the original "cannot be pointed at" phrasing overstated a general limitation. But the *specific tool access available in this environment* cannot construct a working custom bench for it right now — every real attempt was legitimately rejected, confirmed empirically, not assumed. This narrows rather than reverses the ADR's practical conclusion: Option C (a hand-authored genome + pareto-style comparison scored directly against the real validation harness, bypassing `@metaharness/darwin`'s own gated bench format entirely) remains the correct, working approach for tasks with genuine multiple-candidate-design shape (e.g. `opda:Survey`, `opda:Proprietorship`'s conditional Relator minting) — it was never blocked by this finding, since it never depended on the gated bench/evolve path in the first place.

**2026-07-05 (2) — second, deeper re-review: the "no accessible mechanism" claim above was ALSO too strong. Corrected again.** The user challenged again and asked for a source-code-level (not doc-level) re-check. A dispatched agent read the real `@metaharness/darwin@0.8.0` source directly (`packages/darwin-mode/src/bench/suite.ts`, confirmed byte-identical against the actually-installed npm package's `dist/bench/suite.js`) and found the actual mechanics of `taskHash`:

```js
export function hashTasks(tasks) { return sha256(JSON.stringify(canonicalise(tasks))); }
export function makeSuite(id, version, tasks) {
  return { id, version, createdAt: ISOString, taskHash: hashTasks(tasks), tasks };
}
```

`taskHash` is **not a content-authorization lock naming a fixed, sanctioned template** — it is an ordinary hash-on-write, verify-on-read integrity check computed from *whatever tasks array is passed to `makeSuite()`*. It only fails when a suite's stored hash no longer matches its own `tasks` content — exactly what happens when a human or agent hand-edits the JSON text of an already-created suite file without recomputing the hash (which is precisely what both this session's own hand-edit attempt AND, independently, `rupixel/docs/BENCH.md`'s documented "hand-enriched suite... does NOT pass bench verify" case did wrong). `packages/darwin-mode/src/bench/index.ts`'s own doc comment — independently cross-checked via a second `search_ruvnet` call before accepting this, not taken on the first agent's word alone — confirms the design intent precisely: *"suite — immutable, hash-pinned task snapshots (**anti-tampering**)"*, and exports `suite.js`'s functions at the package's public root (`export * from './suite.js'` inside `bench/index.ts`, itself re-exported from the package's top-level `index.ts`) — a real, public, documented API surface, not a private internal.

**The legitimate, tested path**: `import { bench } from '@metaharness/darwin'` and call `bench.makeSuite(id, version, tasks)` / `bench.saveSuite(path, suite)` directly — constructing the suite object programmatically (so the hash is computed correctly from real content at creation time) rather than hand-editing a pre-existing scaffolded JSON file's text after the fact. The agent verified this live (isolated scratch npm install of the real published package, never touching the OPDA repo): built a real OPDA-specific task (a genuine `opda:Survey` gap-closing prompt, `publicTestCommand: "make ci-ontology"`, `hiddenTestCommand: "make rml-test"`, real `allowedMutationFiles` naming real target paths) via `bench.makeSuite(...)`/`bench.saveSuite(...)`, then ran the actual installed CLI binary (`node node_modules/@metaharness/darwin/dist/cli.js bench verify .metaharness/bench.json`) against it: `Suite opda-rml-gaps@1.0.0: 1 tasks, hash OK (f106827fe1fc…)`, exit code 0 — accepted, not tampered.

**Final, settled conclusion**: `metaharness_evolve` genuinely CAN be pointed at this ontology's own real files for real, task-specific gap-closing work, end-to-end — via a short (~30-line) authoring script using the package's own public `bench` library API, not via the `metaharness_bench`/`metaharness_evolve` MCP tool wrappers' own `op`/`repo`/`out`/`suite` parameters alone (which remain, correctly, unable to author custom content — that part of both prior passes was accurate). "No accessible mechanism in this session" was wrong; `Bash` + `npm install @metaharness/darwin` + a short authoring script is squarely within what this session can do. Option C (the hand-rolled genome-comparison pattern used for the `EPCCertificate`/`NearbyFacility` trials, and the one already in flight for `opda:Proprietorship`/`opda:Survey`) remains valid and was never *dependent* on this finding — but it is no longer the only correct answer; a properly-authored `bench.json` driving the real `metaharness_evolve`/`evolve --bench` path is now a confirmed, available alternative for future work of this shape.
