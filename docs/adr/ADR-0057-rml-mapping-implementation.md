...
- **No JSON-shape workaround was found** (pre-declaring the new key with a `null` placeholder in the other item does not help; matching schemas as closely as possible still breaks it as long as any genuinely new key appears at all with 2+ items present). The reliable mitigation: keep JSON arrays that mix genuinely different `oneOf` branch shapes to a single item in the tracked regression fixtures, and verify additional branches via a standalone scratch instance (materialise + spot-check the target triples directly) rather than adding them as further heterogeneous array items to `testdata/03-multi-participant.json`. See the `managedFreeholdOrCommonhold` region's comment in `mapping/opda-pdtf.rml.ttl` for the concrete instance of this.
