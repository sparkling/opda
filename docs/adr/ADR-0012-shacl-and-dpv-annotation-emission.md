...
- **2026-06-01 — Evidence shapes to follow the ODR-0027 §R6 re-model (PENDING).** When the evidence subtypes are retired to coded classification (ODR-0027 §R6 / ADR-0011 pending re-model), the `opda:*CoherenceShape` family (class↔value, `sh:targetClass` on the `…Evidence` subclasses) is **dropped** (no subclasses left to cohere); enforcement rests entirely on the value-keyed `opda:EvidenceTypeValueShape` (value-space gate) + `opda:EvidenceFacetShape` (value-keyed Vouch obligation) — both already `sh:targetSubjectsOf opda:evidenceType`, so they survive the re-model unchanged. The DPV refinement records (`opda-claim-annotations.ttl`) re-point from `targetsKind opda:DocumentEvidence/…` to `opda:Evidence` + an `opda:evidenceType`-keyed `variantValue`. **Status: mandated; implementation PENDING with the ADR-0011 re-model.**
