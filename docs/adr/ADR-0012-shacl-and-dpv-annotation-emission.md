...
- **2026-06-01 — Per-subtype evidence validation EMITTED (Council [session-035](../ontology/odr/council/session-035-evidence-alias-retirement-and-faceted-typing.md)).** session-035 found that the `sh:xone`-on-`evidence.type` dispatch ODR-0009 §"SHACL over the PROV structure" promised — and `opda:Evidence`'s `rdfs:comment` advertised — was **never emitted** (the only evidence shape targeted the supertype `opda:Evidence`, a lone `opda:digest` maxCount). Now closed: `emit-shapes` emits (i) **`opda:EvidenceTypeValueShape`** — the evidence-kind value-space gate (`sh:targetSubjectsOf opda:evidenceType` + `sh:in` over `opda:EvidenceMethodScheme`; the `opda:ownerType` ODR-0024 R6 idiom; entailment-free, SHACL-Core), and (ii) **`opda:VouchEvidenceShape`** — the per-subtype obligation (`sh:targetClass opda:VouchEvidence`, `opda:attestedBy` `sh:minCount 1` `sh:class prov:Agent`, `sh:Violation`). The `opda:Evidence` comment's false "ADR-0012 emits the shape" claim is corrected to name the realised shapes. SHACL-Core throughout (no SHACL-AF / `advanced=True`). All gates + round-trip green.
- **2026-06-01 — Evidence shapes to follow the ODR-0027 §R6 re-model (PENDING).** When the evidence subtypes are retired to coded classification (ODR-0027 §R6 / ADR-0011 pending re-model), the `opda:*CoherenceShape` family (class↔value, `sh:targetClass` on the `…Evidence` subclasses) is **dropped** (no subclasses left to cohere); enforcement rests entirely on the value-keyed `opda:EvidenceTypeValueShape` (value-space gate) + `opda:EvidenceFacetShape` (value-keyed Vouch obligation) — both already `sh:targetSubjectsOf opda:evidenceType`, so they survive the re-model unchanged. The DPV refinement records (`opda-claim-annotations.ttl`) re-point from `targetsKind opda:DocumentEvidence/…` to `opda:Evidence` + an `opda:evidenceType`-keyed `variantValue`. **Status: IMPLEMENTED (2026-06-01)** — `shapes.py` dropped the 3 `*CoherenceShape`s; `opda:EvidenceTypeValueShape` + `opda:EvidenceFacetShape` (both `sh:targetSubjectsOf opda:evidenceType`) carry enforcement unchanged; `annotations.py` re-pointed the 3 DPV refinements (the refinement record IRIs are unchanged, so `test_annotations.py` still passes). All gates + round-trip green.
