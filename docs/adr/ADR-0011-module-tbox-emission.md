...
- **2026-06-01 — Evidence short-name aliases RETIRED (Council [session-035](../ontology/odr/council/session-035-evidence-alias-retirement-and-faceted-typing.md); supersedes the within-engineering option (b) affirmed in the `3d941c4` validation).** The three `owl:equivalentClass` short-name alias classes (`opda:Document`/`opda:ElectronicRecord`/`opda:Vouch`) are removed from `claim.py`. Rationale (8–0–0): a synonymy need mis-encoded as a class-identity axiom — inert under the ODR-0025/0026 closure yet a latent DL-explosion hazard and an active silent-pass (exemplars typed `a opda:Document` matched no emitted shape). Replacement: `skos:altLabel` carries the short name on each canonical `…Evidence` class; the three diagnostic exemplars re-type to the canonical class; the evidence-kind discriminator becomes the governed `opda:evidenceType` facet (`skos:exactMatch`-bound to `opda:EvidenceMethodScheme` per ODR-0011 §8a). Migration was generator-mechanical (3 alias-class blocks dropped + 3 exemplar re-types + facet wiring); all 7 CI gates + 337 pytest + 27 round-trip green; byte-identity re-pinned. The modelling recast (Evidence→RoleMixin, Vouch→Relator) is recorded in the ODR-0009 session-035 amendment.
- **2026-06-01 — Evidence subtypes to be RETIRED to coded classification (ODR-0027 §R6; directing-authority adoption of the hm approach).** ODR-0027 supersedes session-036's keep-the-subclasses disposition: opda:Evidence is a role a document plays, so the three `…Evidence rdfs:subClassOf opda:Evidence` axioms are retired and evidence-kind becomes the `opda:evidenceType` `isMemberOf` coded classification; `opda:AttachedDocument` stays as the one genuine Kind; kind-specific attributes (`opda:attestedBy`…) become role-borne facets (`rdfs:domain opda:Evidence`, documentary). `claim.py` change: drop the 3 subclass declarations + their `skos:exactMatch`-to-subclass + re-home `attestedBy`'s domain to `opda:Evidence`. **Status: IMPLEMENTED (2026-06-01)** — `claim.py` retired the 3 `…Evidence` subclasses + the orphan scheme-concept constants; `CLASSES` now `{Claim, Evidence, AttachedDocument, AssuranceLevel, TrustFramework, VerificationActivity}`; `opda:Evidence` is `owl:Class, opda:RoleMixin`; `opda:AttachedDocument` retained as the one genuine Kind. All 7 CI gates + 337 pytest + 27 round-trip green; byte-identity re-pinned.
