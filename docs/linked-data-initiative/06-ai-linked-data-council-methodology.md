...
For a standards association, the useful framing is that the Council can prepare **technical evidence for real governance**, never substitute for it. The historic `adoption.md` designed a proposal → Working Group → Sub-Committee → AGM handoff, but this repository does not prove those bodies completed that process. In practice, `accepted` was also applied after internal directing-authority decisions, so it must be read as an internal technical status unless independent OPDA governance evidence establishes adoption. The AI panel can do citation, dissent and hard-case analysis; SPDTF's collaborative working groups retain authority over the scheme.
