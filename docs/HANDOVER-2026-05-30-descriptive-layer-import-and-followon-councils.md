...
**Author:** Henrik (with Claude). **Scope:** one session — decided *how* to import the PDTF descriptive layer into the ontology (the prior question the deferred 935-leaf walk exposed), built the sound, gated slice, and ran the four follow-on pattern-councils it spawned. **Status:** **records written; build + site green; the G1 fix and Categories E+D are emitted and committed (2026-05-30); Category-G per-leaf IRIs remain held for the WG.**
...
**Held by decision:** the **188 Category-G permanent IRIs** (→ WG curated pass — the swarm produced the *candidate* set, minted zero G IRIs). *(Update 2026-05-30: Categories **E** — `opda:RiskAssessment` + `opda:PerilScheme`, ODR-0008d — and **D** — the `inclusionStatus` Mode, R4 — have since been emitted, verified green, and committed; only Category G remains held.)*
...
4. **~~Full Category-E/D emission~~ — DONE (2026-05-30):** E (`opda:RiskAssessment` + `opda:PerilScheme`, ODR-0008d) and D (`inclusionStatus` Mode, R4) are emitted, verified (pytest 243 pass / 1 xfail + all four CI gates + site build green), and committed. Three modelling choices are flagged for WG review (RiskIndicatorScheme reuse vs YesNoNotKnownScheme; PerilScheme narrower; `opda:price` vs a MonetaryAmount value type).
...
The records, generator changes, and regenerated TTLs are committed on `main` (not pushed — deploy is via CI on push): `a051332` (ontology arc) + `d44cc47` (UI/site arc). The `descriptive-category-binning.json` report is a gitignored build artefact (regenerated by `categorise-leaves`). Records remain `proposed` pending OPDA WG ratification.
