...
Enrich the existing Understand → Explore → Contribute journeys around a practical outcome: **a participant can help shape the specification by reading a candidate and offering a useful, evidence-aware challenge without learning ontology syntax**. Keep the Harbour Court story, existing distinctions and shared navigation. First explain what the programme is creating; then add a familiar-form-to-connected-model comparison, short reasoned practice, and one guided candidate-page reading. Test those additions before expanding them.
...
| [Contexts and connections][contexts] already explains the established boundaries and rejects organisation-chart boundaries and forced universal definitions. | A single story offers limited practice applying those distinctions in other professions. | Add a few explicitly fictional questions within the documented boundaries, not a boundary-discovery exercise or an asserted map of professional definitions. |
...
1. Which package should be implemented first, and on which exact post-visual-change baseline? Recommended first slice: P0.0, P0.1 and P0.2; then classification and candidate reading.
