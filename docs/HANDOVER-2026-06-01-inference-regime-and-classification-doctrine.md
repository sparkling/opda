...
| `603cb76` | **Inference regime ratified + implemented (partial).** ODR-0025/0026 + ADR-0035/0036/0037 (`accepted`); `config/opda-owl-rl-safe.rules`; `materializeEntailments()` + consistency gate wired into `scripts/fuseki-load.mjs`; ODR-0004 §3a + `opda-inference.md` re-scoped (DL-projection intent superseded). **Plus session-035** evidence work: aliases retired → `skos:altLabel`/`skos:exactMatch`, `opda:evidenceType` facet added, Evidence→RoleMixin / Vouch→Relator, value-keyed shapes. ODR-0009/0011/0026 + ADR-0011/0012 amended. |
