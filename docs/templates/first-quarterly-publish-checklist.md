...
[ADR-0005](../adr/0005-deferred-work-register.md) register item C3 marks the **first quarterly publish** of the Accreditation Directory as the end-of-quarter milestone where the Directory first holds real, member-submitted data — downstream of A1–A4 (Wave 2 content), C1 (build pipeline), and C2 ([VC submission tooling](./vc-submission-spec.md)). This checklist is the runbook for that publish, and intentionally reusable each subsequent quarter so the first run is not a one-off.

The cadence itself sits in [ADR-0004](../adr/0004-accreditation-directory.md) §4: VCs in by the 15th of the quarter-end month; Directory refreshed by the 1st of the following quarter. The timeline below pegs to that.
...
- [ ] OPDA-listed audit-partner roster ([ADR-0005](../adr/0005-deferred-work-register.md) item C4) is current; any new audit partners added since the last publish are reflected in the Trust Registry's audit-attestation allowlist.
...
- [ ] `scripts/smoke-test.mjs` (Playwright) passes on the staging deploy. ***WG decision required:*** does this script exist yet? [ADR-0005](../adr/0005-deferred-work-register.md) E1 leaves it as TBD pending Henrik's next session — if not built, this line stays as a manual checklist instead.
...
5. **What "first publish" means specifically.** [ADR-0005](../adr/0005-deferred-work-register.md) C3 is the *first* time the Directory holds real data. Some checklist items above (e.g. diff against last quarter) only apply from publish 2 onwards. The first run skips them — flag with strikethrough rather than removing the line.

## References

- [ADR-0005](../adr/0005-deferred-work-register.md) §C3 — register entry this checklist operationalises; also §C1 (build pipeline) and §C2 ([VC submission tooling](./vc-submission-spec.md)).
- [ADR-0004](../adr/0004-accreditation-directory.md) §4 — quarterly cadence; §5 — schema; §6 — evidence requirements per score level.
- [ADR-0001](../adr/0001-adopt-dcam-dmbok-elements.md) Wave 2 — the workstreams whose outputs the Directory aggregates.
