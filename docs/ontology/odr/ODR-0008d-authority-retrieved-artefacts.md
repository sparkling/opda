...
>
> **WG-review recommendations (2026-05-30, engineering analysis — not decisions):** (1) **`RiskIndicatorScheme`** → **APPLIED 2026-05-30: `riskIndicator` reuses `YesNoNotKnownScheme`** (value-spaces byte-identical {No, Not known, Yes}; ODR-0022 §1 Category C forbids a duplicate scheme over an existing value-set; the node shape constrains via a literal `sh:in`). The dedicated `RiskIndicatorScheme` is removed; `ActionAlertRatingScheme` (1–5) stays (no existing equivalent). Applying this surfaced and fixed a **latent duplicate declaration**: `opda:riskIndicator` had been declared in *both* `opda-property.ttl` (stale flat-Property home, `rdfs:domain opda:Property`) and `opda-descriptive.ttl` (RiskAssessment Quale, `rdfs:domain opda:RiskAssessment`) — conflicting domains; the stale Property declaration is removed, so `riskIndicator` now lives solely on `opda:RiskAssessment` per ODR-0008d. All four CI gates + site build green. (2) **`PerilScheme` `skos:narrower`** → **sound as-is**: PDTF v3 attests sub-results only as free-text `subCategory` (no enumerated sub-peril value-set), and Rule 4 routes per-result recursion through `opda:hasSubAssessment`; revisit only if a governed sub-peril register appears. (3) **`opda:MonetaryAmount`** → **keep the shared `opda:price` now; defer** a first-class value type to the Category-G monetary-leaf walk (~225 monetary leaves, only `price` emitted today; no currency sub-field in the data). All three honour §4's binding constraint and defer the speculative half.
