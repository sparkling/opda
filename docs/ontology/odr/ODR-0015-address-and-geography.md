...
* **Option A (chosen) — `opda:Address` as a UFO Substance Kind / DOLCE NonPhysicalEndurant, with `opda:addressVariant` quality and class-with-property-shapes structure.** The chosen approach, ratified by Council Session 015.
* **Option B — Address-in-Foundation (Davis's withdraw-conditional position).** Rejected by scope-check majority (7-1): Address has its own identity-criterion work that ODR-0004's Foundation scope (URI + graph separation) cannot carry; routing it into 0004 buries it under namespace concerns.
* **Option C — Routed to Session 006 (the current plan's §4.1 routing).** Rejected by scope-check (8-1): wrong forum (Guizzardi optimising for Kind/Role/Phase, not for INSPIRE/ISO 19160/OS AddressBase relations).
* **Option D — Routed to Session 008 (Property descriptive attributes).** Rejected: orphans the four non-property consumers (participants, evidence issuers, EPC certificate, search authorities).

## Decision Outcome

Chosen option: "`opda:Address` as a UFO Substance Kind / DOLCE NonPhysicalEndurant", because the exemplar evidence (rural-plot INSPIRE-only locatedness) + external-standards alignment (INSPIRE Annex I models Address as a feature; vCard models it as a class; OS AddressBase Plus is record-based) + DPV class-level dispatch together force a Kind reading that the Mode-only framing could not deliver.
...
* **Namespace ratified; record `accepted`.** The inherited ODR-0004 namespace block (Knublauch DA primary demand from S004) is lifted — the `opda:` string was ratified 2026-05-27 (greenfield; no WG) — so ODR-0015 is `accepted`. Generator output for `opda:Address`, `opda:line1` etc. predicates may still carry `dct:status "draft"` as a publication-grade marker, independent of record ratification.
...
* **Re-open triggers (recorded for future sessions):** Allemang DA Q3 trigger: 18 months / zero multi-Property-shared-Address cases / no new consumer query → structured-datatype reverter reconsideration. Davis-from-Scope-Check-1-Q7a (Address-in-Foundation alternative): if URI policy substantively changes in ODR-0004 such that Address modelling moves there, ODR-0015 may be superseded.
...
flowchart LR
    ...
    T["opda:Address<br/>addressVariant: title"]:::process
    M["opda:Address<br/>addressVariant: marketing"]:::process
    I["opda:Address<br/>addressVariant: inspire"]:::process
    P["opda:Property"]:::success
    X["NO owl:sameAs<br/>across variants"]:::warning
...
flowchart TD
    ...
    T1{"Title-extents<br/>in scope?"}:::warning
    T2{"LLC1 search-area<br/>polygons in scope?"}:::warning
    T3{"INSPIRE polygon<br/>direct ingest?"}:::warning
    T4{"Search-radius<br/>queries needed?"}:::warning
    GEO["Full GeoSPARQL<br/>encoded geometry<br/>admitted"]:::success
    IF --> T1
    IF --> T2
    IF --> T3
    IF --> T4
...
flowchart LR
    ...
    ODR5["ODR-0005<br/>Property &amp; Land Identity"]:::process
    ODR3["ODR-0003<br/>Programme"]:::process
    ODR17["ODR-0017"]:::process
    ODR18["ODR-0018"]:::process
    ODR15["ODR-0015<br/>Address &amp; Geography"]:::warning
    ODR6["ODR-0006<br/>Agents &amp; Roles"]:::success
    ODR8["ODR-0008<br/>Property Descriptive Attrs"]:::success
    ODR9["ODR-0009<br/>Claims &amp; Evidence"]:::success
    ODR12["ODR-0012<br/>Data-Governance Layer"]:::success
    ODR13["ODR-0013<br/>SHACL Validation"]:::success
