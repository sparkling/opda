...
* **No register — let the cut options live only in the session-022 transcript** — they rot and get re-proposed every revisit (the failure mode this programme already hit twice); a transcript is not a standing, trigger-indexed gate.
* **Build the enhancements now (adopt PROF + DCTAP-artefact + materialised tags)** — the over-engineering the governance directive rejected: it wraps the SHACL that already does the job, for consumers that do not exist.
* **Track these in ADR-0005 §G (the deferred-work register)** — wrong corpus (these are ontology-artefact representation, ODR-side) and §G is the retired ontology-*programme* register, not the form layer; cross-listing there would split the form-layer decision across two records.
* **Fold into ODR-0010 as "future work"** — buries a standing, trigger-gated register inside a ratified mechanism record; a dedicated record keeps ODR-0010's normative mechanism clean and makes the deferral list independently citable.
