...
* **Option A (chosen) — Three-class Property pattern (`opda:Property` + `opda:LegalEstate` + `opda:RegisteredTitle`).** Three UFO Substance Kinds committed to DOLCE Endurant, keyed operationally by SHACL/DASH uniqueness on UPRN with graceful degradation; chosen because it is checkable, degrades gracefully for new-builds and pre-first-registration cases, avoids irreversible cross-context inference propagation, and gives `RegisteredTitle` instances a class-level discriminator for ICO Subject Access processing.
* **Option B — One `opda:Property` Kind with alternative identifiers + SHACL co-reference (Kendall, FIBO-LEI pattern).** Rejected: strains when physical referent and legal interest diverge (multi-title flat, commonhold), risking re-conflation of physical and legal identity in one class.
* **Option C — Two classes: Property (physical) + LegalEstate (Allemang).** Rejected: does not give the Land Registry's *record* its own identity, which Hendler/Guizzardi argue is a third thing with its own lifecycle.
* **Option D — Three classes + RegisteredTitle (Hendler / Guizzardi).** Same as chosen — this was the three-class approach evaluated alongside Options B and C; the exemplars justify the third class.
* **Option E — Two endurants with explicit ICs: Site/BuiltStructure + LegalEstate (Guarino).** Rejected: most demanding to author and validate; the time-indexed `realises`/`vests-in` relation introduces OWL-Time dependency the simpler splits avoid.
* **Option F — `owl:hasKey` on the rigid Kind as primary.** Rejected: only *licenses inference* and is inert for a consumer whose record has no UPRN; does nothing for the new-build / first-registration case.
* **Option G — `owl:sameAs` across UPRN surfaces.** Rejected: propagates every context's properties onto every other, irreversibly under inference (unanimous rejection).

## Decision Outcome

Chosen option: "Option A — three-class Property pattern", because it is the only option that gives the unregistered-house case a coherent answer, gives `RegisteredTitle` instances a class-level discriminator for ICO Subject Access processing, and avoids irreversible cross-context inference propagation.

Adopt the **three-class Property pattern** ratified by [Council Session 005](./council/session-005-property-land-identity-crux.md) (Phase 2 gate; B2 pilot consensus-mode hive-mind/byzantine; Queen Guarino, DA Allemang withdrew on 8 of 8 questions): three UFO Substance Kinds committed to DOLCE Endurant — **`opda:Property`** (physical Substance Kind; IC = spatial-material continuity with legal-record discontinuity override), **`opda:LegalEstate`** (legal-institutional Substance Kind; IC = rights-bundle persistence; may be unregistered), **`opda:RegisteredTitle`** (registry-record Substance Kind; IC = title-number lineage; carries distinct published-personal-data PII regime under HMLR open-register). Keyed operationally by SHACL/DASH uniqueness on UPRN with graceful degradation; UPRN succession captured by reified `opda:UPRNSuccessionEvent` materialised into the validation report via a SHACL-AF rule at `sh:Info` severity; joined via co-reference (never `owl:sameAs`); UPRN modelled as UFO Quality / contingent scheme-scoped identifier under `prov:wasDerivedFrom` succession.
...
**Added by [Council Session 005](./council/session-005-property-land-identity-crux.md) — Allemang DA full withdrawal (8 of 8 questions); Davis + Cagle held-as-live dissent on Q5 preserved.**
...
* **ODR-0009 (Claims, Evidence & Provenance), ODR-0010 (Overlay Profile Mechanism), ODR-0012 (Data-Governance Layer), ODR-0013 (SHACL Validation & Severity) UNAFFECTED** (their gates are upstream-independent of ODR-0005; they inherit specific load-bearing inputs — see More Information).
