...
**Artefact-level `status: accepted` blocked** on WG ratification of the `opda:` namespace string (inherited from ODR-0004 Knublauch DA primary demand). Until ratification, generator output for `opda:Property`, `opda:LegalEstate`, `opda:RegisteredTitle`, and all property predicates carries `dct:status "draft"` in the ontology header. When ODR-0004 moves to `accepted`, ODR-0005 follows.
...
- **Namespace block carries forward.** ODR-0005 stays `status: proposed` until WG ratifies the namespace string per ODR-0004's Knublauch DA primary demand. Generator output carries `dct:status "draft"`; downstream `pattern` ODRs inherit the same status discipline through their `depends-on` chains.
