...
- The namespace string `https://w3id.org/opda/#` is now ratified (WG decision 2026-05-27, [Session 003b](./council/session-003b-namespace-wg-decision.md); engineering realisation [ADR-0006](../../adr/ADR-0006-w3id-opda-ontology-namespace.md)). Resolution depends on the W3C PICG redirect to `https://openpropdata.org.uk/ontology/`; the OPDA web team commits to keeping the redirect target serving the canonical TTL artefacts.
- Versioning scheme (calendar vs semantic mirroring schema 3.4.0) remains WG-owned: `owl:versionIRI` cannot be frozen until the WG rules.
