...
* **Full OWL 2 DL classification (HermiT/Pellet/Konclude), the dormant `opda-inference.ttl` path** — rejected: unsound identity-merging and triple explosion in a master-data ontology, computationally heavy, explicitly out of the directed scope, and never built.
* **Live Jena `ja:reasoner` (InfModel) at query time, as the primary mechanism** — rejected: Jena 6's assembler cannot wrap an InfModel around a multi-graph TDB2 dataset without losing named-graph GSP upload (the sibling project hit exactly this and disabled its live reasoner), and it loses asserted-vs-inferred provenance. Materialisation (ADR-0035) is preferred; the InfModel remains a documented fallback.
* **No entailment regime (status quo) — rely on per-call pyshacl `inference="rdfs"`** — rejected: leaves "which triples are entailed?" undefined, couples entailment to one validator's flag, and applies a *broader* (domain/range-including) RDFS than is safe.
* **Full RDFS, including `rdfs:domain`/`range` entailment** — rejected: multi-domain properties mis-type individuals (R2); this is the direct cause of the EPCCertificate false positive.
* **Defer RDF 1.2 triple-term syntax to stay `rdflib`-parseable (the sibling's path)** — rejected: it lets the weaker tool dictate the data model. opda already standardises on Jena (`riot` parses triple terms; `jena-shacl` validates SHACL 1.2), so the `rdflib` gap is not a constraint opda accepts — the fix is to drop the `rdflib` dependency, not downgrade the model (R5).
