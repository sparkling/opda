...
**Re-open trigger (recorded per §Consequences template; confirmed with Queen + tightened via Isaac's three-branch test):** *Re-open the status modelling when a future PDTF schema version or named consumer (AML onboarding / conveyancing case-management / lender) introduces role-specific status beyond the four envelope values `Proposed|Invited|Active|Removed`.* But the **resolution depends on which of Isaac's three cases fires**, and only one of them reopens the single-scheme commitment itself:
- **Case 1 (role-specific subset of states):** resolve with `skos:Collection` + `skos:member` (non-exclusive) + overlay `sh:in` (ODR-0010) — NOT a scheme split. A split would break ODR-0011 §1a's exactly-one-`skos:inScheme` IC (Cagle invariant) the moment a state is shared.
- **Case 2 (role-specific transitions / edges):** resolve with a role-keyed SHACL/SHACL-AF transition rule (ODR-0013/0017) over the shared node vocabulary — NOT a scheme split. A scheme enumerates nodes, not the transition graph.
- **Case 3 (genuine role-divergent definitions):** this — and only this — reopens the single-scheme commitment. Absent in both schema and emitted artefact (`opda-vocabularies.ttl:2188–2218`), so unfired.

Until then, P stands and per-role schemes would be modelling fiction.
