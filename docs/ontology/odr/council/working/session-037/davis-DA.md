...
**Residual — now WITHDRAWN; full convergence (Baker's governance ruling + Allemang's httpRange-14 layer).** My residual was: the wall is a path segment under one authority, not a second coordinate authority — *unless* the governance model needs `/pdtf/` and `/harness/` separately delegable. Both halves are now resolved:

- **Baker ruled on delegation (the governance call was his):** opda's model does NOT need the two separately delegable. The DCMI delegation test — a coordinate authority is earned only when the segments could plausibly diverge in *stewardship* (different maintainer / change-control body / independently transferable) — fails here, because `/harness/` is *definitionally* opda-about-pdtf (its ADRs, ODRs, fixtures, graphs are authored/versioned/retired BY opda AS pdtf's steward). Co-stewardship is structural, not incidental. So the second authority is unearned and a **path segment under one `/opda/` redirect** is correct.
- **Allemang supplied the affirmative reason the path is *required*** (not merely sufficient): the three-graph separation makes the cut queryable in a *loaded* triplestore, but the URI must serve a consumer holding only a cold string. `rdf:type` can't route a cold GET (circular — fetch-to-learn-how-to-fetch) and graph membership isn't in the string; **only a path segment is in the string at routing time** (Cool URIs / httpRange-14). The path earns its keep at the HTTP layer the graph cut structurally cannot reach: differential cache/stability/redirect policy (`/pdtf/` → stable host, long TTL; `/harness/` → churny host). So position (3) — one authority, `/harness/` as a path — is the *floor*, and "no path at all" is wrong. I withdrew that flirtation.

Crucially, nothing is given back: the boundary-in-the-URI is fully carried by the PATH distinction — a consumer reads "normative" off the segment identically whether it's a coordinate root or a path. Path-vs-coordinate was purely a redirect-surface choice, zero effect on what the URI signals. **So Q5 closes single-voiced:** Baker's binding citation-test + default-to-harness + promote-only-by-council + never-demote + one-directional dependency, realised as a **path under one `/opda/` authority**. Grounded both sides: Baker's DCMI delegation reasoning, Allemang's httpRange-14, my *Linked Data Patterns* Hierarchical URIs. This is recorded in the ADR-0006 amendment as Q5 disposition §4 (Allemang's realisation note).
...
| Q5 standard-vs-physical split | REVISE | FOR Baker's citation-test + default-to-harness, realised as `/harness/` PATH under one `/opda/` authority | FULL WITHDRAW — single-voiced (Baker delegation ruling + Allemang httpRange-14); residual resolved, not held |
...
**Net (after cross-talk).** Both of my primary structural attacks were answered by grounded counters and I moved on both — which is the point of the role, not its failure. I affirm the persistence-bearing, change-resistant decisions (Q1, Q3-core, Q6). On **Q4 (flat namespace)** I conditionally withdrew after Pandit supplied the missing finding (no independent-versioning/adoption driver at opda's scale) — flat is sound *provided the reopening criterion is recorded* so the carve stays non-breaking. On **Q5 (the standard/harness partition)** I fully withdrew. Baker hardened the boundary into a *falsifiable, binding citation-test* with a default-to-harness / promote-only-by-council / never-demote fallback (answering the governance objection and correcting me on the DCMI never-reassign asymmetry), then ruled on delegation: opda does not need `/pdtf/` and `/harness/` separately delegable, so the cut is a **path segment under one `/opda/` authority** — collapsing my last residual. Allemang supplied the affirmative httpRange-14 reason the path is *required* (only a path segment is in a cold URI string at routing time; `rdf:type`/graph-membership can't route a cold GET). Q5 closes single-voiced and stronger than as drafted. On **Q2/Q3** I HOLD with narrow re-open triggers rather than attack: the decisions are defensible but their stated rationales are incomplete (slash buys nothing under a single document; a `versionInfo` literal is not a citable release — a versioned release URI should be reserved in `/harness/`). The through-line holds: the scheme is right where it buys persistence and earns its structure; my residuals are about *not minting a distinction twice* and *recording reopening triggers so nothing is foreclosed irreversibly*.
