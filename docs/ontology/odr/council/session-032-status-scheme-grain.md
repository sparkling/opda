...
## Held-as-live dissents + re-open triggers

1. **Participant-status SET-test** (Guizzardi + Guarino, co-signed; Kendall's preserved per-role case). Re-open per-role participant-status schemes **only** if a role-play *within a single transaction* has a status **partition** (its SET of phase universals) differing from `{Proposed,Invited,Active,Removed}` — a different **SET**, not a different current value. Remedy ladder: (a) divergent value still sharing the role-play identity regime → role-scoped `skos:Collection` under the single scheme (default); (b) only if the identity regime itself diverges → a separate sub-scheme (rare). *Illustration:* a Conveyancer "Disinstructed" fires it only if irreducible to `Removed` and meaningless for buyer/seller. **Also** (Kendall): a future overlay/trust-framework introducing a role-specific status/stage enum **as actual data**, or a named consumer query needing role-partitioned "Active", re-opens it.
2. **Milestone-scheme subkind scope** (Guizzardi, held-as-live; does NOT touch participant-status). A Remortgage/Transfer-only transaction surfacing a milestone phase absent from (or a sale phase inapplicable to) the standard sale lifecycle → the **milestone** scheme gets a subkind-scoped treatment (`skos:Collection` or overlay profile), NOT the participant-status scheme. This is per-Transaction-**subkind**, not per-role.
