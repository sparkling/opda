...
- ~~**Session 015 — ODR-0015 Address & Geography** (Reduced Council, Phase 2.6 gate)~~. **CLOSED** by [session-015](./session-015-address-and-geography.md). `opda:Address` commits to UFO Substance Kind; 5 of 8 questions withdrawn by DA, 3 conceded, 1 held-as-live (Q3 class-structure with named re-open trigger). Second `kind: pattern` ODR to discharge under A9. Downstream ODR-0006 + ODR-0008 unblocked at deliberative level. **B2 pilot second site observation supports EXTEND-CAUTIOUSLY; awaiting S011 Q8 as third site for EXPAND threshold.**
- **B3 pilot — Session 011 Q8** — UFO meta-category per scheme; `consensus-mode: hive-mind/typed-output`. Substrate landed; A9 resolved (per-scheme UFO category + IC over named hard cases now MUST-level for `kind: pattern`). Pilot unblocked. **Third pilot site for two-artefact discipline EXPAND threshold.**
- **Session 006 — ODR-0006 Agents & Roles** (Phase 3a gate). Unblocked by S005 + S015 close. Inherits 3-class commitment + IC discipline (SHACL primary; no `owl:sameAs`; PROV-O succession) + `opda:Address` class with `vcard:Address` reuse for participant contact.
- **Session 007 — ODR-0007 Transactions & Lifecycle**. Unblocked by S005 close; inherits 3-class commitment as load-bearing.
- **Session 008 — ODR-0008 Property Descriptive Attributes** — **DEFERRED** until S005's 3-class commitment crystallises into the leaf-to-class mapping (Kendall+Davis joint amendment from S005). S015 closure does not release S008's deferral; the cardinality is the gating dependency.
