...
> **Amendment — Council [session-050](./session-050-adopt-semantic-modelling-owl-coverage.md) (2026-06-17, 5–0): Q5's `founds`/`mediates` carrier ruling is SUPERSEDED.** This Q5 reasoning — "can't add the entailment axiom and keep 'NEVER reasoned'", so type-pin `founds`/`mediates` in SHACL `sh:class` **instead of** `rdfs:domain`+`rdfs:range` — rested on a **false premise**: it reasoned about OPDA from standard OWL entailment (Hendler's "everything-becomes-a-Person" MUST-warning), which OPDA's frozen 7-rule closure **never performs** (ODR-0026 §R2 model-but-don't-evaluate; ODR-0029 §R1; ADR-0035 adds zero domain/range triples). Hendler himself **withdrew that premise as applied to OPDA** in [session-048](./session-048-owl-documentary-domain-range-r2-set.md). Under session-050 (adopting semantic-modelling's OWL coverage), **`founds`/`mediates` carry documentary `rdfs:domain`+`rdfs:range` AND keep their SHACL `sh:class` shapes** — the "Design-time, NEVER reasoned" commitment is **preserved, not deleted** (documentary domain/range and "never reasoned" are complementary, not in tension), and ODR-0029/0030/0031 are **not** overridden. **What STANDS from Q5:** Option D (SHACL-only) rejected; the rangeless-AND-shapeless ban; the two-graph separation; and **bearer disjunctions in SHACL `sh:or`, NOT `owl:unionOf`** (session-050 agreed on rejecting the boolean class constructor — it adds documentary repeated-`rdfs:domain` "any-of" *alongside* the `sh:or`). **What is SUPERSEDED:** the "type-pinned in SHACL `sh:class`, NOT `rdfs:domain`+`rdfs:range`" carrier choice, and the "fail a not-universally-true `rdfs:domain`/`range`" gate limb (inverted — documentary "any-of" is now required). Routed via ODR-0032 §R1/§R2/§Decision-detail/§Confirmation amendments + ADR-0049.
