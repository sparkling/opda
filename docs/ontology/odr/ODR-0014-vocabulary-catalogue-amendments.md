...
* **Option A (chosen) — Record amendments as a partial-supersession amendment ODR (ODR-0014).** The original chosen approach: a separate amendment record preserving per-session provenance.
* **Option B — Edit ODR-0002 in place.** Rejected: erases the provenance of *why* each tier moved and *which* deliberation moved it, collapsing two distinct authorship events into one undated table.
* **Option C — A fresh full catalogue ODR superseding ODR-0002 wholesale.** Rejected: falsely implies the whole catalogue was re-deliberated when only a handful of entries were touched, and discards ODR-0002's still-valid survey reasoning for the untouched tiers.

## Decision Outcome

Chosen option: "Record amendments as a partial-supersession amendment ODR", because the amendment-ODR pattern was itself superseded by Scope-Check 1 (2026-05-26) — which ruled the convention is not the right approach — and the amendments now live in ODR-0002's `## Change log`-inside-`## Rules` pattern.
