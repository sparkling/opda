...
### Execution status (2026-05-27 — Phase 0 + Phase 1 + Phase 2 + Phase 2.5 + Phase 2.6 Council gates all cleared)
...
| 2.6 | **S015 — ODR-0015 Address & Geography (Phase 2.6 gate)** | **Reduced Council; two-artefact discipline (B2-pilot second site)** | **Guizzardi** | **Allemang (5 withdrawn / 3 conceded / 1 held-as-live on Q3)** | **Second `kind: pattern` ODR to discharge under A9.** Q1 3-0 FOR `opda:Address` as UFO Substance Kind / DOLCE NonPhysicalEndurant (**revises S005 Q6 Mode-only stance** on exemplar evidence + external-standards alignment); Allemang DA withdrew on 3 named consumer queries + DPV class-level dispatch SHACL case. Q2 3-0 FOR five-rule IC. Q3 **2-1 FOR class with property shapes**; **Allemang DA held-as-live dissent** with named re-open trigger ("18 months / zero multi-Property-shared-Address cases / no new consumer query"). Q4 3-0 FOR three alignments (INSPIRE-as-contingent-identifier; vCard superclass; OS AddressBase via `dct:source`). Q5 3-0 FOR GeoSPARQL deferral + `opda:hasGeometry` interface + four admission triggers. Q6 3-0 FOR two-tier SHACL shape (`sh:Info` cross-variant; `sh:Warning` same-variant). Q7 3-0 FOR class-level `dpv-pd:Address` + ODR-0012 routing. Q8 3-0 PASS with per-exemplar verdict walkthrough + amendments. ODR-0015 amended: §Decision rewritten for Kind; new §Operational specifications 2a/3a/3b/4a/5a/6a/7a/8a; §Consequences records Q3 held-as-live dissent. **status:proposed retained** per namespace block. Downstream ODR-0006 + ODR-0008 unblocked at deliberative level. **B2 pilot second-site observation: structured tally caught Q3 dissent + DA scorecard as machine-readable; supports EXTEND-CAUTIOUSLY recommendation. Awaiting S011 Q8 as third site for EXPAND threshold.** **`pattern`-extraction candidate flagged:** PROV-O succession reaches third citing site (ODR-0005 §6a + ODR-0009 + ODR-0015 §4a); spawn shared `contingent-identifier-succession` pattern record if ODR-0006 produces fourth. |

**Council Phase 1 + Phase 2 gates have cleared substantively.** Formal status-accepted close (and unblocking of downstream module sessions' `status: accepted`) awaits **WG namespace ratification** — see "Open WG decisions" below.
...
### Pending sessions (next in priority order)

The remaining 7 active sessions (S006, S007, S009, S010, S012, S013; S008 deferred-on-cardinality; S016 deferred-until-trigger) follow per §5 phasing. S005 + S011 + S015 closed 2026-05-27 (Phase 2 + Phase 2.5 + Phase 2.6 gates cleared).

1. **S006 — Agents & Roles** (Phase 3a gate for S007/S009/S012). Unblocked by S005 + S011 + S015 close. Inherits 3-class commitment + IC discipline (SHACL primary; no `owl:sameAs`; PROV-O succession) for Person/Organisation identity + `opda:Address` class (S015) + SKOS framework (S011 — role = Role label, participantStatus = Phase label). **Between-session prep done** (Person/Org exemplars committed `7f46029`).
2. **S007 — Transactions & Lifecycle** (Phase 3b). Unblocked by S005 close; inherits 3-class commitment + S011 Method/plan code framework (sellersCapacity).
3. **S008 — Property Descriptive Attributes** (Phase 3b). **DEFERRED per S005 Q5+Q8 Kendall+Davis joint amendment** — leaf-to-class mapping depends on 3-class crystallising. S011 closure provides Quale-in-Region framework for descriptive schemes (councilTaxBand/currentEnergyRating/builtForm/ownershipType) but does NOT release deferral on cardinality.
4. **S009 → S012 → S010 → S013** (Phases 4/5/6) per §5.
5. **S016 — W3C VC / DID Compatibility Layer** (Phase 7 deferred) — activates only on named trigger.

**Follow-up author-only sessions flagged (post-S011):**

- **ODR-0001 EXPAND amendment** — records that two-artefact discipline (narrative + structured tally + DA scorecard mechanical-check) is now DEFAULT across all remaining sessions; session-level B2/B3 pilot consensus-mode labels retired; update §Format tiers / §Substrate operations.
- **Pattern-extraction spawn** — spawn shared `opda-shacl-af-quality-rules` `pattern` ODR; ODR-0005 §6a + ODR-0009 + ODR-0015 §4a + ODR-0011 §5a all `implements:` it (fourth citing site reached at S011).
- **Phase-3.5 audit session** (post-S006/S007/S008): Q3 cross-vocabulary mapping (S011 deferral) + SSSOM re-open trigger (S002 Q11).

The 7 remaining active sessions (S006/S007/S009/S010/S012/S013 + S008-deferred; S016 deferred-until-trigger) plus 3 follow-up Author-only sessions follow §5 phasing.
...
**Phase 1 — Foundation spike (gate).** ✅ **Council gate cleared (2026-05-27); formal close awaits WG namespace ratification.**
...
**Phase 2 — Identity crux (gate). [PILOT — Scope-Check 2 B2]** ✅ **Council gate cleared (2026-05-27); formal close awaits WG namespace ratification.**
...
**Phase 2.5 — Enumeration substrate.** ✅ **Council gate cleared (2026-05-27); formal close awaits WG namespace ratification.**
...
**Phase 2.6 — Address gate (new — Scope-Check 1 Q7a).** ✅ **Council gate cleared (2026-05-27); formal close awaits WG namespace ratification.**
