...
test('desktop sidebar and nested tree controls work', async ({ page }) => {
  ...
  const leftDuringTransition = await geometry();
  expect(leftDuringTransition.mainLeft).toBeCloseTo(initial.mainLeft, 1);
  expect(leftDuringTransition.mainRight).toBeCloseTo(initial.mainRight, 1);
  await expect.poll(async () => (await geometry()).sidebarWidth).toBeCloseTo(44, 1);
  ...
  await expect(page.locator('#section-navigation')).toHaveJSProperty('inert', true);
  await expect(page.locator('#section-navigation')).toHaveCSS('opacity', '0');
  await page.locator('#sidebar-collapse').click();
  await expect(body).not.toHaveClass(/sidebar-collapsed/);
  await expect.poll(async () => (await geometry()).sidebarWidth).toBeCloseTo(initial.sidebarWidth, 1);
  await expect(page.locator('#section-navigation')).toHaveJSProperty('inert', false);
  await expect(page.locator('#section-navigation')).toHaveCSS('opacity', '1');
  await toc.locator('#toc-collapse').click();
  await expect(body).toHaveClass(/toc-collapsed/);
  const rightDuringTransition = await geometry();
  expect(rightDuringTransition.mainLeft).toBeCloseTo(initial.mainLeft, 1);
  expect(rightDuringTransition.mainRight).toBeCloseTo(initial.mainRight, 1);
  await expect.poll(async () => (await geometry()).tocWidth).toBeCloseTo(44, 1);
  ...
  await expect(toc.locator('ul')).toHaveJSProperty('inert', true);
  await expect(toc.locator('ul')).toHaveCSS('opacity', '0');
  await toc.locator('#toc-collapse').click();
  await expect.poll(async () => (await geometry()).tocWidth).toBeCloseTo(initial.tocWidth, 1);
  await expect(toc.locator('ul')).toHaveJSProperty('inert', false);
