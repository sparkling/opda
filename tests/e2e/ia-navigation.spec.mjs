...
test('public entry is the work-in-progress landing and points into the knowledge base', async ({ page }) => {
  const clean = watchRuntime(page);
  await visit(page, '/');
  await expect(page.locator('.cs-art')).toHaveCount(1);
  await expect(page.locator('.cs-signin')).toHaveAttribute('href', '/programme');
  await expect(page.locator('.cs-signin')).not.toHaveAttribute('href', '/home');
  clean();
});
