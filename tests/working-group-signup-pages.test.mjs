...
test('global header promotes the canonical working-group sign-up route', async () => {
  ...
  assert.match(baseCss, /\.app-header\s*\{[^}]*grid-template-rows:\s*auto auto calc\(var\(--target-min\) \+ var\(--space-4\)\)/su);
  assert.match(baseCss, /\.global-nav-panel\s*\{[^}]*height:\s*calc\(var\(--target-min\) \+ var\(--space-4\)\)/su);
