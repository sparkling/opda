...
test('shared navigation exposes visible focus, state and 44px targets', async () => {
  ...
  assert.match(header, /<div class="global-nav-panel"[\s\S]*<nav class="global-nav"[\s\S]*GLOBAL_NAVIGATION_ITEMS\.map[\s\S]*<HeaderPreviewControls[\s\S]*showScaleControl[\s\S]*identityId="app-header-identity"[\s\S]*initialScale=\{24\}[\s\S]*initialSpaceBelow=\{0\}[\s\S]*initialIcon="common-boundary"[\s\S]*initialPalette="clay-indigo"[\s\S]*\/>/u);
