...
test('shared navigation exposes visible focus, state and 44px targets', async () => {
  ...
  assert.match(base, /@media \(min-width: 96\.0625rem\) \{\s*:root \{ --header-height:\s*9rem; \}/u);
  ...
  assert.match(base, /\.app-header__title\s*\{[^}]*margin-block-end:\s*calc\(-1 \* var\(--space-2\)\)/su);
