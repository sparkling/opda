...
test('temporary selectors expose full preview cards and persist palettes and icons', async () => {
  ...
  assert.match(tuningSelector, /label: 'List item spacing'[\s\S]*property: '--domains-item-spacing', min: 0, max: 64, value: initialPanelItemSpacing/u);
  ...
  assert.match(campaign, /\.wg-hero-journey \.wg-process li\s*\{[^}]*padding:\s*var\(--domains-item-spacing, var\(--space-4\)\) 0;/su);
  assert.match(campaign, /\.wg-hero-journey \.wg-process li:last-child\s*\{\s*padding-bottom:\s*0;\s*\}/u);
  ...
  assert.match(client, /function bindHeaderPaletteSelector\(life\)/u);
  assert.match(client, /function bindHeaderPalettePagination\(life\)/u);
  ...
  assert.match(client, /bindHeaderPalettePagination\(life\);/u);
  assert.match(client, /function bindHeaderIconSelector\(life\)/u);
  assert.match(client, /function bindHeaderPreviewControls\(life\)/u);
  assert.match(client, /async function loadHeaderPreviewControls\(life\)[\s\S]*searchParams\.has\('config'\)[\s\S]*fetch\(source, \{ credentials: 'same-origin', signal: life\.signal \}\)[\s\S]*loader\.replaceWith\(controls\)/u);
  ...
  assert.match(client, /await loadHeaderPreviewControls\(life\);[\s\S]*syncHeaderConfigurationMode\(\);[\s\S]*bindHeaderPaletteSelector\(life\);/u);
  assert.match(client, /labelDataKey:\s*'iconNumber'/u);
  assert.match(client, /function bindIdentityHeadingControls\(life\)/u);
  assert.match(client, /function syncRenderedHeaderHeight\(\)/u);
  assert.match(client, /life\.observe\(new ResizeObserver\(life\.guard\(syncRenderedHeaderHeight\)\)\)/u);
  ...
  assert.match(client, /bindHeaderPreviewControls\(life\);/u);
  assert.match(client, /function bindThemeToggle\(life\)\s*\{[\s\S]*querySelectorAll\('\.theme-toggle'\)[\s\S]*life\.markBound\(button, 'themeBound'\)/u);
  assert.doesNotMatch(client, /getElementById\('theme-toggle'\)/u);
  assert.match(client, /function bindHeaderPreviewSelector\(config, life\)/u);
  ...
  assert.match(client, /if \(config\.closeOnSelect === false\) return;[\s\S]*target\.closest\(config\.optionSelector\)[\s\S]*life\.frame\(function \(\) \{ closeSelectorChain\(selector\); \}\)/u);
  assert.match(client, /const nextTarget = event\.relatedTarget;[\s\S]*nextTarget instanceof Node && !selector\.contains\(nextTarget\)/u);
  assert.match(client, /life\.listen\(document, 'pointerdown'[\s\S]*selector\.open && target instanceof Node && !selector\.contains\(target\)/u);
