...
test('reply renderers contain the required operational guidance', () => {
  ...
  assert.match(renderMembershipConfirmation('Alex', ['Technology Working Group']), /has been added/i);
  assert.match(renderSubmissionConfirmation('Alex', ['dictionary.xlsx', 'schema.json']), /dictionary\.xlsx/);
  assert.match(renderSubmissionConfirmation('Alex', ['dictionary.xlsx', 'schema.json']), /schema\.json/);
});
