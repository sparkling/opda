...
test('reply renderers contain the required operational guidance', () => {
  ...
  assert.match(access, /organisation(?:'s|&#39;s)? security polic/i);
  ...
  for (const reply of [clarification, access, membership, submission]) {
    ...
    assert.match(reply, /^<div[^>]*>/i);
    assert.match(reply, /<p\b[^>]*>/i);
    assert.match(reply, /margin:/i);
  }
  assert.match(clarification, /<ul\b[^>]*>/i);
  assert.match(membership, /<li\b[^>]*>/i);
  assert.match(renderSubmissionConfirmation('<Alex>', ['schema<script>.json']), /&lt;Alex&gt;/);
  assert.match(renderSubmissionConfirmation('<Alex>', ['schema<script>.json']), /schema&lt;script&gt;\.json/);
  assert.doesNotMatch(renderSubmissionConfirmation('<Alex>', ['schema<script>.json']), /<script>/i);
});
...
test('harness manifest freezes the model and the scheduled prompt is fail-closed', () => {
  ...
  assert.match(prompt, /createReply/i);
  assert.match(prompt, /contentType.*HTML/is);
