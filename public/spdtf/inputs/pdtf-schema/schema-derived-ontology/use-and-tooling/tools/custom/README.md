# custom `opda-gen` — the composed schema-derived ontology reference
...
`tools/custom/`. It is the **woven schema-derived ontology section itself** — the
...
LLM prose must supply. The custom output is woven into the schema-derived ontology
pages and guarded by the two-tier doc-drift CI gate (ADR-0041 §Confirmation,
B1):
