...
export const WORKING_GROUPS = Object.freeze([
  ...
  },
  {
    ...
    aliases: ['conveyancing'],
    workspace: {
      status: 'provisioned',
      teamId: 'aab9afb4-a959-4b3a-be5b-6fbd92ecf22d',
      siteUrl: 'https://openpropertydataassociation.sharepoint.com/sites/ConveyancingSourceIntake',
      teamUrl: 'https://teams.microsoft.com/l/team/19%3a_zZicFW0oJReC4seUosrcdU_NSkosNnH8DMp0vMBIWs1%40thread.tacv2/conversations?groupId=aab9afb4-a959-4b3a-be5b-6fbd92ecf22d&tenantId=143540d4-4fbc-4005-882a-29656cd01a36',
      contributorGroupPrefix: 'Conveyancing Contributors - ',
      indexGroup: 'Conveyancing Organisation Index Users',
    },
  },
  {
    id: 'estate-agency',
    ...
    workspace: {
      status: 'provisioned',
      teamId: '0bea5db6-f0d5-46ba-8785-cfb1c4393805',
      siteUrl: 'https://openpropertydataassociation.sharepoint.com/sites/EstateAgencySourceIntake',
      teamUrl: 'https://teams.microsoft.com/l/team/19%3abEeGItoblFaHbJ4jv824umyxGbXMTPN9ajEoQfkbMuI1%40thread.tacv2/conversations?groupId=0bea5db6-f0d5-46ba-8785-cfb1c4393805&tenantId=143540d4-4fbc-4005-882a-29656cd01a36',
      contributorGroupPrefix: 'Estate Agency Contributors - ',
      indexGroup: 'Estate Agency Organisation Index Users',
    },
  },
  {
    ...
    workspace: {
      status: 'provisioned',
      teamId: '3d45ce47-aa03-4331-84ae-285fad735896',
      siteUrl: 'https://openpropertydataassociation.sharepoint.com/sites/SurveyingValuationSourceIntake',
      teamUrl: 'https://teams.microsoft.com/l/team/19%3aezfKYsrJrKYZLAF-yy-4OHy0D0xD5kQvZK9VQF8Pdts1%40thread.tacv2/conversations?groupId=3d45ce47-aa03-4331-84ae-285fad735896&tenantId=143540d4-4fbc-4005-882a-29656cd01a36',
      contributorGroupPrefix: 'Surveying and Valuation Contributors - ',
      indexGroup: 'Surveying and Valuation Organisation Index Users',
    },
  },
  {
    ...
    workspace: {
      status: 'provisioned',
      teamId: 'f1281e5f-8097-4ece-8226-7be1864cff13',
      siteUrl: 'https://openpropertydataassociation.sharepoint.com/sites/PropertyDataServicesSourceIntake',
      teamUrl: 'https://teams.microsoft.com/l/team/19%3aMlFTOJZA08QsDMrkinrpxpTwsao4mpTWwPE5Rm_ZHgU1%40thread.tacv2/conversations?groupId=f1281e5f-8097-4ece-8226-7be1864cff13&tenantId=143540d4-4fbc-4005-882a-29656cd01a36',
      contributorGroupPrefix: 'Property Data Services Contributors - ',
      indexGroup: 'Property Data Services Organisation Index Users',
    },
  },
  {
    ...
    workspace: {
      status: 'provisioned',
      teamId: 'c9e907e8-5fdf-4165-adba-d3b2a7a466ff',
      siteUrl: 'https://openpropertydataassociation.sharepoint.com/sites/PropertyTechnologySourceIntake',
      teamUrl: 'https://teams.microsoft.com/l/team/19%3ajusLihthBkrjaHq68Udr0dSn_KTz3GiURCXxbpNo-jo1%40thread.tacv2/conversations?groupId=c9e907e8-5fdf-4165-adba-d3b2a7a466ff&tenantId=143540d4-4fbc-4005-882a-29656cd01a36',
      contributorGroupPrefix: 'Property Technology Contributors - ',
      indexGroup: 'Property Technology Organisation Index Users',
    },
  },
  {
    id: 'dbt-smart-data',
...
function greeting(name) {
  return `Hello ${escapeHtml(nonEmptyString(name, 'recipient name', 160))},`;
}
...
function itemList(items) {
  ...
  return `<ul style="margin:0 0 22px;padding:0 0 0 24px;">${rows}</ul>`;
}

function replyDocument(name, content) {
  return `<div style="max-width:680px;font-family:Arial,Helvetica,sans-serif;font-size:15px;line-height:1.6;color:#242424;">${paragraph(greeting(name))}${content}<div style="margin:26px 0 18px;padding:14px 16px;border-left:4px solid #e5b632;background:#fff4cf;color:#4a4232;font-size:13px;line-height:1.5;"><strong>AI-generated response</strong><br>${AI_RESPONSE_NOTICE}</div><p style="margin:0;">Kind regards,<br><strong>Smart Property Data Trust Framework</strong></p></div>`;
}

export function renderClarification(name) {
  const content = paragraph('Thanks for the request. Before I add the user, please confirm which working group or groups they should join:')
    + itemList(WORKING_GROUPS.map(({ name: groupName }) => groupName));
  return replyDocument(name, content);
}

export function renderAccessGuidance(name) {
  const content = paragraph('I’m sorry you are having trouble accessing Teams or SharePoint. If your organisation&#39;s security policy permits it, you can try a personal email address or personal device for Teams participation. Please do not bypass your organisation&#39;s security controls.')
    + paragraph('Teams gives you access to working-group discussions, organised reply threads, updates, and review of model drafts. If SharePoint remains unavailable, you can send authorised source material to this email address as attachments and OPDA will place it in the correct organisation folder.')
    + paragraph('SharePoint uploads still require an approved company-domain account; this does not prevent an invited personal account from participating in Teams.');
  return replyDocument(name, content);
}

export function renderMembershipConfirmation(name, groupNames) {
  const groups = groupNames.map((groupName) => nonEmptyString(groupName, 'working-group name', 160));
  return replyDocument(name, paragraph('The requested user has been added and their invitation email has been sent for:') + itemList(groups));
}

export function renderSubmissionConfirmation(name, resourceNames) {
  const resources = resourceNames.map((resourceName) => nonEmptyString(resourceName, 'resource name', 255));
  const content = paragraph(`Thank you. The following resource${resourceNames.length === 1 ? ' has' : 's have'} been added to your organisation&#39;s SharePoint folder:`)
    + itemList(resources)
    + paragraph('You can send further authorised material to this email address as attachments if SharePoint is unavailable.');
  return replyDocument(name, content);
}
