...
export function toManualRoute(absPath: string): string | null {
  ...
  const slug = m[1]
    ...
    .replace(/\/$/, '');
  return slug ? `/development/inputs/pdtf-schema/schema-derived-ontology/model-views-by-audience/${slug}` : '/development/inputs/pdtf-schema/schema-derived-ontology/model-views-by-audience';
}

/** Remark plugin: rewrite relative `.md` links into Astro `/development/inputs/pdtf-schema/schema-derived-ontology/model-views-by-audience/…` routes. */
