 ...
 * `/pdtf` dereference route and the `/pdtf-schema/schema-derived-ontology` typed indexes — no runtime query.
 *
...
/** The /pdtf-schema/schema-derived-ontology/concepts-and-architecture/contexts page URL for a module, or null if it isn't one of the 7. */
export const contextUrl = (m: string | null | undefined): string | null =>
  (isContext(m) ? `/pdtf-schema/schema-derived-ontology/concepts-and-architecture/contexts/${m}` : null);
 ...
 * minted opda: term IRIs, so they live at /pdtf-schema/schema-derived-ontology/concepts-and-architecture/contexts/{id}.)
 */
