...
Version 1.1 · adopted 16 August 2026 · updated 10 September 2026 · scope: opda.org.uk application and its standalone design-system presentation.
...
At 1536px (96rem) and below, the seven-destination global site navigation becomes a non-modal disclosure without independently removing the paired identity. Medium layouts retain both identity lines in a 120px header; at 960px and below the 64px narrow header omits the framework line and selector. Above that
...
seven-destination audience/task directory, a participation action and the footer. The directory comes from the canonical global-navigation registry and
...
- **Header actions:** a shared flex group aligns the transparent secondary “Become a member” action immediately before the primary “Join a working group” action at the content-right edge. Both and “Sign in” share `.btn--compact` padding and natural height. Placement belongs to the header grid, not button-local size or positional overrides; the compact disclosure keeps the membership/join pair right-aligned in normal flow.
...
- **Section heading:** `CampaignSectionHeading.astro` owns the campaign eyebrow, title and optional lead/media slots. Its uppercase Source Sans 3 small-text eyebrow and display title are one stack with an 8px gap; section/panel/compact variants change only the title role. Split introductions use 45:55 columns and a 32px gutter, stacking below 64rem. Do not recreate local eyebrow fonts, negative margins or heading grids.
...
- **Campaign card grid:** `CampaignCardGrid.astro` owns the shared spacing, padding, type, borders and responsive columns for motivation, policy, groups, participation and evidence. Every grid has 48–64px clearance above it, declared once in the component. Group cards read a 28px heading (`--text-2xl`), 4:1 editorial drawing and exactly two expanded, justified paragraphs. A visually hidden native link covers the whole card and carries the selected group to the signup form, with one keyboard stop and visible hover/focus feedback; no separate action label repeats the heading. `CampaignThemeImage.astro` owns light/dark source switching for every campaign illustration. The form keeps its concise descriptions.
  Colour-rich pen-and-ink/watercolour drawings remain active only in their assigned image-set-3 placements in `public/images/join/set-3/`: six 4:1 working-group pairs and three 3:1 section-introduction pairs follow the site's selected mode. They are historical production assets, not templates or a default medium for new work. Motivation, participation and professional judgement use `CampaignSectionHeading` with `.wg-section-media`, showing the 1536×512 landscape siblings uncropped above their paragraphs. ADR-0078 records the earlier prompts and generation provenance; `public/images/join/set-3/judgement.manifest.json` records the judgement pair's native-generation prompts and edit lineage. Older section images, original photos (set 1) and first drawings (set 2) remain archived, never reassigned to a different placement. All are local compressed WebP assets with lazy loading; images are decorative and never replace the label or practical scope. Group cards use a three-column maximum; other grids may use four. Preserve ordinary HTML reading order and avoid modelling terminology in audience choices. Campaign sections share 32–48px vertical padding; inset boundary panels use 32px padding rather than stacking large section gaps. The trust panel uses one editorial introduction over a responsive two-column boundary register. The form-to-footer gap stays 32px.
...
Generative imagery is never used in the OPDA identity. OPDA uses a governed **subject-led illustration family**, not one default medium. Every brief names the placement, its learning or promotion purpose, and one concrete subject and action before selecting a technique. Shared quality comes from expressive craft, the OPDA light/dark papers and gold, petrol, violet, rose and coral palette—not from repeating a watercolour treatment or generic property workshop. Generate and approve the light composition first, then edit that exact image for dark mode without changing geometry or using CSS inversion. Keep text in HTML, record purpose, prompt and asset provenance, preserve established export dimensions, and never use illustration as evidence or precise technical notation. The canonical briefing and pairing templates live on `/design-system`.

Choose deliberately among **editorial watercolour or gouache** for a humane situation, **architectural cutaway or ink-and-coloured-pencil** for a physical distinction, **short sequential ink narrative** for a change or decision over time, **risograph or screenprint** for a declarative opener, and **paper craft** for a material metaphor. Other well-justified techniques may be used when the subject requires them. Do not combine techniques as a random decorative collage. Every page, card and section placement receives a unique concept and composition; an illustration may never be reused elsewhere. Its matched light/dark pair and resolution renditions count as one asset family. The dated six-direction study and production manifests retain historical prompts but do not override this rule. Replacement artwork uses new filenames and preserves earlier files and dimensions. Lower-page art is selectively placed and shallower than the page header, within the same left-aligned 1024px reading measure; preserve the complete composition without stretching or cropping.

The supplied AI poster is removed. The sole recurring decorative motif is a 4px amber rule used for headlines, active navigation and footer structure; it does not imitate or redraw the logo. The root homepage and `/join` intentionally share one static radial/diagonal campaign-hero background treatment. The method figure, presentation tile and subject-led illustrations carry editorial meaning and are not recurring identity motifs.
...
2. Standard targets are at least 44×44px; inline prose links and shared compact labelled controls are deliberate exceptions, with compact controls remaining above the 24px minimum.
