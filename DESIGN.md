...
The yellow primary working-group action finishes on the content track's right edge. It uses the shared inset-button template so its complete 44px target remains aligned while its standard yellow surface clears the divider by 4px. The action and destination labels share
