...
- **Campaign card grid:** `CampaignCardGrid.astro` owns the shared spacing, padding, type, borders and responsive columns for motivation, policy, groups, participation and evidence. Every grid has 48–64px clearance above it, declared once in the component. Group cards read a 28px heading (`--text-2xl`), 4:1 editorial drawing and exactly two expanded, justified paragraphs. A visually hidden native link covers the whole card and carries the selected group to the signup form, with one keyboard stop and visible hover/focus feedback; no separate action label repeats the heading. `CampaignThemeImage.astro` owns light/dark source switching for every campaign illustration. The form keeps its concise descriptions.
  Colour-rich pen-and-ink/watercolour drawings are active as image set 3 in `public/images/join/set-3/`: six unchanged 4:1 working-group pairs and three 3:1 section-introduction pairs follow the site's selected mode. Motivation, participation and professional judgement use `CampaignSectionHeading` with `.wg-section-media`, showing the 1536×512 landscape siblings uncropped above their paragraphs. ADR-0078 records the earlier prompts and generation provenance; `public/images/join/set-3/judgement.manifest.json` records the judgement pair's native-generation prompts and edit lineage. Older section images, original photos (set 1) and first drawings (set 2) remain available. All are local compressed WebP assets with lazy loading; images are decorative and never replace the label or practical scope. Group cards use a three-column maximum; other grids may use four. Preserve ordinary HTML reading order and avoid modelling terminology in audience choices. Campaign sections share 32–48px vertical padding; inset boundary panels use 32px padding rather than stacking large section gaps. The trust panel uses one editorial introduction over a responsive two-column boundary register. The form-to-footer gap stays 32px.
...
Choose the illustration medium for the lesson, not automatically for the section. Watercolour remains the familiar default; **architectural cutaways** use hand-drafted ink and coloured pencil to explain physical distinctions, and **sequential ink narratives** use crisp linework and flat pastel gouache for short human stories. A **risograph editorial** may provide a distinct manifesto or section opener, with restrained halftone and overprint rather than watercolour washes. Paper craft and nocturne pencil remain alternatives, not a mandatory mixture across every page. The local six-direction image study records the original briefs; the production asset manifests retain each selected prompt and theme pair. Illustrations never establish exact relationships, approval or factual evidence: those belong in accessible text and diagrams. Replacement artwork uses new filenames and preserves all previous illustration files. Lower-page art is selectively placed and shallower than the page header, within the same left-aligned 1024px reading measure; preserve the complete composition without stretching or cropping.
