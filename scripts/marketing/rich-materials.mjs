...
<desc id="${slug}-desc">Practitioners bring evidence to working groups, who review meaning to inform the draft specification. ${escapeHtml(pack.example)}</desc>
...
export function renderLinkedInPreview(pack, voice, logoBytes, heroBytes, infographic) {
...
<title>${escapeHtml(pack.label)} · ${voice === 'opda' ? 'OPDA' : 'Organisation'} LinkedIn campaign</title>
<style>
...
<h1>Three conversations that invite a useful contribution.</h1>
${source.posts.map((post, index) => `<article><p class="kicker">Post ${index + 1} of ${source.posts.length}</p><h2>${escapeHtml(post.title)}</h2>
${index === 0 ? `<img class="art" src="${hero}" alt="${escapeHtml(pack.hero.alt)}">` : index === 1 ? `<figure style="margin:24px 0 32px">${infographic}<figcaption class="caption">Professional evidence informs review of the draft specification. This is work in development, not an adopted requirement.</figcaption></figure>` : `<p class="quote">Your professional experience belongs in the discussion.</p>`}
