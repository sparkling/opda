...
DATA = ROOT / 'docs/previews/evidence-2-5'
TARGET = ROOT / 'docs/previews/evidence-shared-meaning-11-styles.html'
manifest = json.loads((DATA / 'prompts.json').read_text())
...
for style in manifest['styles']:
    ...
    note = reviews.get(style['id'], 'Generated; visual review pending.' if image.exists() else 'Generation paused; no preview yet.')
    if image.exists():
        ready += 1
...
{visual}
<div class="description"><p>{esc(style['description'])}</p><p class="review"><strong>Review:</strong> {esc(note)}</p><p class="risk">{esc(style['risk'])}</p>
<details><summary>Exact API image prompt</summary><pre>{prompt_html}</pre></details></div>
</article>''')
...
<title>From evidence to shared meaning — 20 API style previews</title>
<style>
...
'''
page += f'<p><strong>{ready} / 20 images generated.</strong> Requested API model: <code>{esc(manifest["model"])}</code>; quality: high; size: auto, with a 1280 × 512 / 2.5:1 target in the prompt. Individual review notes distinguish verified output from unresolved defects.</p>'
if manifest.get('generationState', '').startswith('paused'):
    page += '<p class="review"><strong>Generation paused: model provenance unresolved.</strong> The API requests name Sunburst 2.5, but returned files record softwareAgent version 2.0. These are not certified 2.5 executions. No further images will be generated until the discrepancy is resolved.</p>'
...
'''
page += '<details><summary>API model and output provenance</summary><p>The native OpenAI API recognises the requested Sunburst model ID. Requests explicitly use that ID. The pilot image reports gpt-image version 2.0 in its C2PA softwareAgent field; the mapping of that field to the API model remains unresolved. Automatic sizing returned 1983 × 793 for the pilot; gallery renditions are reduced proportionally to at most 1280 pixels wide. Original outputs are retained.</p></details>'
if manifest.get('investigation'):
    investigation = manifest['investigation']
    page += '<details open><summary>Source and impact of the mismatch</summary><p>' + esc(investigation['causeBoundary']) + '</p><ul>' + ''.join('<li>' + esc(item) + '</li>' for item in investigation['impact']) + '</ul><p>' + esc(investigation['vendorFixStatus']) + '</p><p>' + esc(investigation['localCorrection']) + '</p><ul>' + ''.join(f'<li><a href="{esc(url)}">{esc(url)}</a></li>' for url in investigation['sources']) + '</ul></details>'
