...
export const PDTF1_TOOL_REFRAMES = Object.freeze([
  Object.freeze({
    ...
    reason: 'Names the canonical PDTF 1.0 extracted-ontology section instead of the retired /ontology route.',
  }),
  Object.freeze({
    ...
    reason: 'Clarifies retired route history and names the canonical extracted-ontology composition.',
  }),
  Object.freeze({
    ...
    reason: 'Replaces retired /ontology naming with the canonical extracted-ontology hierarchy.',
  }),
  Object.freeze({
    ...
    reason: 'Updates retired /ontology links to canonical extracted-ontology terminology.',
  }),
]);
...
export function composePdtf1ToolReframeReceipt(baseline, accepted) {
  ...
  if (baseline.count !== accepted.count || before.size !== baseline.count || after.size !== accepted.count
    ...
    throw new Error('PDTF 1.0 tool family changed outside the reviewed file set');
  }
  const reframedFiles = PDTF1_TOOL_REFRAMES.map((entry) => {
    ...
    if (baselineRecord?.sha256 !== entry.baselineSha256
      ...
      throw new Error(`PDTF 1.0 tool reframe bytes changed: ${entry.path}`);
    }
